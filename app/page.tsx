...
import { ArrowLeft, ArrowRight, ArrowUp, AudioLines, ChevronRight, Diamond, Flag, Ghost, Maximize2, Pause, Play, RotateCcw, Share2, Snowflake, Shirt, Target, Trophy, Volume2, VolumeX } from 'lucide-react';
...
import { CURRENT_RULES_VERSION, TowerEngine, freshControls, MODE_LABELS, type GameMode, type Snapshot, type RunReplay } from '@/lib/tower-engine';
import { bestGhost, GHOST_STORAGE_KEY, readGhost, startGhostRun, type TowerGhost, type GhostRecord } from '@/lib/tower-ghost';
...
export default function Home() {
  ...
  function startRun(e: TowerEngine, selectedMode: GameMode) {
    resetInput();
    ghost.current = null;
    if (challengeRef.current) startChallengeRun(e, challengeRef.current, selectedMode);
  ...
  function begin(selectedMode = mode) {
    ...
    audio.current?.setPaused(false); setGame(engine.current.snapshot()); setHelp(false); setChallengesOpen(false); tone('jump'); canvas.current?.focus({ preventScroll: true });
  }
  ...
  function menu() { ghost.current = null; setRace(null); engine.current?.menu(); resetInput(); if (engine.current) setGame(engine.current.snapshot()); }
  ...
  useEffect(() => {
    ...
    if (ghostBest.current?.replay.version !== CURRENT_RULES_VERSION) ghostBest.current = null;
    ...
    import('@/lib/tower-world').then(async ({ TowerWorld }) => {
      ...
      try {
        ...
        unregisterTools = registerGameTools(e, { start: selected => { startRun(e, selected); audio.current?.setPaused(false); setHelp(false); setGame(e.snapshot()); canvas.current?.focus(); }, pause: () => { resetInput(); e.togglePause(); setGame(e.snapshot()); } });
        const animate = (now: number) => {
          ...
          for (const event of events) {
            ...
            if (event.type === 'over') {
              ...
              const nextGhost = e.version === CURRENT_RULES_VERSION ? bestGhost(ghostBest.current, e) : ghostBest.current;
  ...
  return <main className={`game-shell state-${game.status} mode-${game.mode} ${challenge ? 'friend-run' : ''}`}>
    ...
    {game.status === 'ready' && <>
      <section className={`title-screen ${challenge || challengeError ? 'has-challenge' : ''}`}><div className="eyebrow"><span /> REACH FOR THE IMPOSSIBLE</div><h1>ICY<br /><span>TOWER</span></h1><div className="subtitle"><i /> F R O S T B O U N D <i /></div>{challenge ? <div className="friend-invite"><span>FRIEND CHALLENGE</span><strong>Beat floor {challenge.floor}.</strong><p>{challenge.score.toLocaleString()} points · {MODE_LABELS[challenge.mode]}<br />Same tower. Your turn.</p><Button variant="ghost" onClick={leaveChallenge}>Leave challenge</Button></div> : challengeError ? <p className="friend-link-error" role="alert">{challengeError}</p> : <p>One more jump.<br />A little closer to the sky.</p>}<Button className="start-button" onClick={() => begin()} disabled={!ready || !!error}><Play size={17} fill="currentColor" />{ready ? challenge ? 'ACCEPT CHALLENGE' : 'BEGIN THE ASCENT' : 'ENTERING THE TOWER…'}<ArrowRight size={19} /></Button><span className="enter-hint">{mode === 'party' ? 'Springs · low gravity · crystal double jumps · ' : ''}or press <kbd>ENTER</kbd></span><span className="touch-hint">{mode === 'party' ? 'Springs · low gravity · crystals grant double jumps' : 'Hold a direction · tap jump with your other thumb'}</span><div className="menu-actions"><Button variant="ghost" onClick={() => setHelp(true)}>How to play <ChevronRight size={13} /></Button><span /><label className="mode-picker" htmlFor="game-mode">Mode<NativeSelect disabled={!!challenge} id="game-mode" aria-label="Game mode" value={mode} onChange={event => { const next = event.target.value as GameMode; setMode(next); if (engine.current) { engine.current.start(next, engine.current.seed, CURRENT_RULES_VERSION); engine.current.status = 'ready'; setGame(engine.current.snapshot()); } }}><NativeSelectOption value="arcade">Classic</NativeSelectOption><NativeSelectOption value="party">Party</NativeSelectOption><NativeSelectOption value="practice">Practice</NativeSelectOption></NativeSelect></label></div><div className="collection-menu extra-menu-actions"><Button variant="ghost" className="challenges-menu-button" onClick={() => setChallengesOpen(true)}><Target size={16} /> Quick challenges</Button><Button variant="ghost" className="leaderboard-menu-button" onClick={openWardrobe}><Shirt size={16} /> Outfits</Button><Button variant="ghost" className="leaderboard-menu-button" onClick={openLeaderboard}><Trophy size={16} /> Leaderboard <ChevronRight size={14} /></Button></div><div className="personal-best"><Flag size={13} /><span>{MODE_LABELS[mode].toUpperCase()} BEST</span><strong>{best.floor.toString().padStart(3, '0')} <small>FLOORS</small></strong></div><div className="ghost-menu-note"><Ghost size={16} /><span>{challenge ? 'Leave the challenge to race your ghost.' : mode !== 'arcade' ? 'Switch to Classic to race your ghost.' : ghostFloor !== null ? `Race your best · Floor ${ghostFloor}` : 'Finish a Classic climb to create your ghost.'}</span></div></section>
      <aside className="challenges-preview"><QuickChallenges challenges={game.challenges} preview /></aside>
    </>}
    {active && <><section className="score-hud"><span className="eyebrow">FLOOR</span><strong>{game.floor.toString().padStart(3, '0')}</strong><div className="score-number">{game.score.toLocaleString()} <small>PTS</small></div><div className="run-readout" aria-label="Run time and tower pace"><span aria-label="Elapsed time">{formatTime(game.time)}</span>{game.mode !== 'practice' && game.pace.level > 0 && <span>· Pace {game.pace.level}</span>}</div><div className="height-readout"><ArrowUp size={13} /> {game.height} m</div><div className="gem-count"><Diamond size={13} /> {game.gems}</div>{race && <div className={`ghost-race ${race.beaten ? 'ghost-beaten' : ''}`}><span><Ghost size={16} /> YOUR BEST · {race.floor}</span><strong>{race.beaten ? 'Best floor beaten!' : race.finished ? 'Ghost finished' : race.lead === 0 ? 'Neck and neck' : `${Math.abs(race.lead)} m ${race.lead > 0 ? 'ahead' : 'behind'}`}</strong><small>{race.finished || race.beaten ? 'Keep climbing for a new record' : 'Racing your previous climb'}</small></div>}{game.mode === 'party' && <div className="party-hud" data-power-active={game.doubleJumpTime > 0}><strong>PARTY · LOW GRAVITY</strong><span>{game.doubleJumpTime > 0 ? `DOUBLE JUMP · ${Math.ceil(game.doubleJumpTime)}s` : 'CRYSTALS GRANT DOUBLE JUMPS'}</span>{game.doubleJumpTime > 0 && <small>{game.doubleJumpReady ? 'PRESS JUMP AGAIN IN MIDAIR' : 'LAND TO RECHARGE'}</small>}<small>Pink springs launch you higher</small></div>}{challenge && <div className="friend-target"><span>{game.floor > challenge.floor ? 'Floor beaten!' : `Beat floor ${challenge.floor}`}</span><small>{game.score > challenge.score ? 'Score beaten!' : `${challenge.score.toLocaleString()} pts to beat`}</small></div>}</section><div className={`combo-hud ${game.combo >= 3 ? 'visible' : ''}`}><span>{game.combo >= 15 ? 'UNSTOPPABLE' : game.combo >= 8 ? 'ON FIRE' : 'KEEP IT GOING'}</span><strong>{game.combo}<small>COMBO</small></strong><div className="combo-track"><i style={{ transform: `scaleX(${game.comboTime / 3.8})` }} /></div></div><div className="speed-meter"><span>MOMENTUM</span><div>{Array.from({ length: 12 }, (_, i) => <i key={i} className={game.speed / 8.4 * 12 > i ? 'filled' : ''} />)}</div><small>{game.speed > 6 ? 'SUPER JUMP READY' : 'BUILD SPEED TO JUMP HIGHER'}</small></div>{game.stormDistance < 4 && game.status === 'playing' && <div className="storm-warning"><ArrowUp size={15} /> THE FROST IS CATCHING UP</div>}{game.status === 'playing' && <fieldset className="touch-controls" aria-label="Touch game controls"><fieldset className="touch-move" aria-label="Movement"><Button aria-label="Move left" aria-pressed={touchPressed.left} data-control="left" {...touchEvents}><ArrowLeft /></Button><Button aria-label="Move right" aria-pressed={touchPressed.right} data-control="right" {...touchEvents}><ArrowRight /></Button></fieldset><Button className="touch-jump" aria-label="Jump" aria-pressed={touchPressed.jump} data-control="jump" {...touchEvents}><ArrowUp /><span>JUMP</span></Button></fieldset>}</>}
    {(game.status === 'paused' || game.status === 'over') && <div className="overlay"><section className="result-card"><Snowflake className="result-mark" size={35} strokeWidth={1} /><span className="eyebrow">{game.status === 'paused' ? 'A MOMENT IN THE QUIET' : 'THE TOWER WILL WAIT'}</span><h2>{game.status === 'paused' ? 'Catch your breath.' : 'One more climb?'}</h2><p>{game.status === 'paused' ? 'Your ascent is right where you left it.' : `${MODE_LABELS[game.mode]} mode · ${game.mode === 'practice' ? 'Unranked climb' : 'Separate ' + MODE_LABELS[game.mode].toLowerCase() + ' rankings'}`}</p>{challenge && <p className="friend-result">{game.floor > challenge.floor ? 'Floor beaten!' : game.floor === challenge.floor ? 'Floor matched — climb one higher to win.' : `Challenge: beat floor ${challenge.floor}.`}<br />{game.score > challenge.score ? 'Score beaten!' : `Score to beat: ${challenge.score.toLocaleString()}`} · {MODE_LABELS[challenge.mode]}</p>}<div className="result-stats"><div><small>FLOOR</small><strong>{game.floor.toString().padStart(3, '0')}</strong></div><div><small>SCORE</small><strong>{game.score.toLocaleString()}</strong></div><div><small>BEST COMBO</small><strong>{game.bestCombo}<em>×</em></strong></div></div><p className="run-summary">Time {formatTime(game.time)} · {game.mode === 'practice' ? 'Practice pace off' : `Pace ${game.pace.level}`}</p><QuickChallenges challenges={game.challenges} compact />{game.status === 'over' && game.mode === 'arcade' && <p className="ghost-result"><Ghost size={18} />{ghostUnavailable ? 'This climb exceeded the ghost recording limit.' : newGhost ? 'New ghost ready. Race this climb next run.' : race ? `Ghost to beat: floor ${race.floor}. Try again.` : 'Reach a floor and finish to record your ghost.'}</p>}<Button className="start-button" onClick={() => game.status === 'paused' ? pause() : begin()}>{game.status === 'paused' ? <Play size={16} /> : <RotateCcw size={16} />}{game.status === 'paused' ? 'CONTINUE ASCENT' : challenge ? 'RETRY CHALLENGE' : 'CLIMB AGAIN'}<ArrowRight size={18} /></Button>{game.status === 'over' && <Button className="leaderboard-result-button" variant="outline" onClick={openLeaderboard}><Trophy size={16} />{game.mode !== 'practice' && game.floor > 0 ? 'Submit score & leaderboard' : 'View leaderboard'}</Button>}{game.status === 'over' && game.floor > 0 && <Button className="leaderboard-result-button" variant="outline" onClick={shareRun}><Share2 size={16} /> Challenge a friend</Button>}<Button className="back-menu" variant="ghost" onClick={openWardrobe}><Shirt size={14} /> Outfits</Button><Button className="back-menu" variant="ghost" onClick={menu}>Return to the tower</Button></section></div>}
    <Dialog open={challengesOpen} onOpenChange={setChallengesOpen}><DialogContent className="result-card help-card challenges-dialog"><DialogTitle>Make this climb count.</DialogTitle><DialogDescription>Complete these quick challenges in a single run. Progress resets when you start again.</DialogDescription><QuickChallenges challenges={game.challenges} preview /><Button className="start-button" disabled={!ready || !!error} onClick={() => begin()}>BEGIN THE ASCENT <ArrowRight size={18} /></Button></DialogContent></Dialog>
    <Dialog open={help} onOpenChange={setHelp}><DialogContent className="result-card help-card"><span className="eyebrow">THE ART OF THE ASCENT</span><DialogTitle>Find your rhythm.</DialogTitle><DialogDescription className="sr-only">Controls and rules for climbing the tower.</DialogDescription><div className="instruction"><span><kbd>A</kbd><kbd>D</kbd></span><div><strong>Move with momentum</strong><p>Hold the left or right touch button, or use A / D or the arrow keys. Slide across the direction pad to turn. Ice is slippery — steer early.</p></div></div><div className="instruction"><kbd>SPACE</kbd><div><strong>Jump. Land. Repeat.</strong><p>Tap JUMP with your other thumb while holding a direction. Release and tap again for the next jump. On a keyboard, use Space, W or ↑. Build speed for higher jumps and star-shaped spins.</p></div></div><div className="instruction"><ArrowRight size={25} /><div><strong>Jump from the walls</strong><p>While airborne beside a wall, release and tap JUMP to push up and away, even at low speed. Land or jump from the opposite wall before boosting from that wall again. Fast automatic rebounds still work. Older friend challenges use the original rebounds.</p></div></div><div className="instruction"><Diamond size={25} /><div><strong>Keep your chain alive</strong><p>Land on higher floors within 3.8 seconds. Keep a combo going to leave a colorful star trail. Collect crystals and skip floors to raise your score.</p></div></div><div className="instruction"><ArrowUp size={25} /><div><strong>Stay above the frost</strong><p>The frost starts rising at floor 5 and gets faster every 30 seconds, with no speed cap. Time and pace appear below your score. Pausing freezes both. Practice removes automatic scrolling; older friend challenges keep their original pace.</p></div></div><div className="instruction"><Diamond size={25} /><div><strong>Party mode</strong><p>Float in low gravity. Pink spring platforms launch you automatically. Collect a pink crystal for 8 seconds of double jumps: press jump again in midair, once per landing. Party has its own rankings and personal best.</p></div></div><div className="instruction"><Ghost size={25} /><div><strong>Race your ghost</strong><p>Your highest completed Classic climb returns as a translucent rival on the same tower. Score, then time, break floor ties. Ghosts are saved in this browser; Party and Practice runs do not replace them.</p></div></div><Button className="start-button" onClick={() => begin()}>LET’S CLIMB <ArrowRight size={18} /></Button></DialogContent></Dialog>
