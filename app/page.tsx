...
import { ArrowLeft, ArrowRight, ArrowUp, AudioLines, ChevronRight, Diamond, Flag, Ghost, Maximize2, Pause, Play, RotateCcw, Share2, Snowflake, Trophy, Volume2, VolumeX } from 'lucide-react';
...
export default function Home() {
  ...
  function startRun(e: TowerEngine, selectedMode: GameMode) {
    ghost.current = !challengeRef.current && selectedMode === 'arcade' && ghostBest.current ? new TowerGhost(ghostBest.current) : null;
    resetInput();
    if (challengeRef.current) startChallengeRun(e, challengeRef.current, selectedMode);
    else e.start(selectedMode, ghost.current?.record.replay.seed ?? Math.floor(Math.random() * 2 ** 30));
  ...
  return <main className={`game-shell state-${game.status}`}>
    ...
    {leaderboardOpen && <LeaderboardDialog open={leaderboardOpen} onOpenChange={setLeaderboardOpen} run={submissionRun} initialMode={mode === 'party' ? 'party' : 'arcade'} />}
