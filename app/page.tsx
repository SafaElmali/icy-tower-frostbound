...
export default function Home() {
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => {
      ...
      try {
        ...
        saved.progress = advanceProgress(saved.progress, { ...bestRef.current, combo: 0 });
        saveProfile(saved);
      } catch { setStorageAvailable(false); }
    });
  ...
  return <main className={`game-shell state-${game.status}`}>
    ...
    {leaderboardOpen && <LeaderboardDialog open={leaderboardOpen} onOpenChange={setLeaderboardOpen} run={submissionRun} outfit={profile.equipped} />}
