...
export function registerGameTools(engine: TowerEngine, actions: { start: (mode: GameMode) => void; pause: () => void }) {
  ...
  const tools: Tool[] = [
    ...
    { name: 'start_tower_run', title: 'Start tower run', description: 'Start a fresh tower climb. Replaces the current run. Classic arcade includes rising frost; party adds springs, low gravity, and crystal-powered double jumps with separate rankings; practice removes the timed chase.', inputSchema: { type:'object',properties:{mode:{type:'string',enum:['arcade','party','practice']}},required:['mode'],additionalProperties:false }, annotations:{readOnlyHint:false,untrustedContentHint:false}, execute: input => { const mode = (input as {mode?:unknown})?.mode; if(mode!=='arcade'&&mode!=='party'&&mode!=='practice') throw new Error('mode must be arcade, party, or practice'); actions.start(mode); return engine.snapshot(); } },
