...
export function graphicsFailureCode(error: unknown, fallback: string): string {
  ...
  if (error.name === 'GraphicsRecoveryTimeoutError')
    return 'context_recovery_timeout';
  if (error.name === 'AbortError') return 'load_aborted';
  if (
    error instanceof RangeError &&
    /array buffer|allocation|memory/i.test(error.message)
  )
    return 'allocation_failed';
  if (
    /Failed to fetch|Load failed|NetworkError|dynamically imported module|Importing a module script failed/i.test(
      error.message,
    )
  )
