...
import { MAX_REPLAY_FRAMES, MAX_REPLAY_SEGMENTS, TowerEngine, replayMode, type RankedMode, type RunReplay } from './tower-engine.ts';
...
export function verifySubmission(input: unknown): LeaderboardEntry & { mode: RankedMode } {
  ...
  if (!replay || (replay.version !== 1 && replay.version !== 2 && replay.version !== 3 && replay.version !== 4 && replay.version !== 5) || !Number.isInteger(replay.seed) || replay.seed < 0 || replay.seed > 0xffffffff || !Array.isArray(replay.moves) || !replay.moves.length || replay.moves.length > MAX_REPLAY_SEGMENTS) throw new LeaderboardError('This run cannot be verified. Start a new ranked run.');
