...
export type LeaderboardEntry = { id: string; name: string; score: number; floor: number; combo: number; duration: number; createdAt: string; outfit?: Outfit };
...
export function verifySubmission(input: unknown): LeaderboardEntry {
  ...
  return { id, name, outfit: normalizeOutfit(outfit), score: engine.score, floor: engine.floor, combo: engine.bestCombo, duration: Math.round(engine.time * 1000), createdAt: new Date().toISOString() };
}
