...
export function bestGhost(current: GhostRecord | null, engine: TowerEngine): GhostRecord | null {
  ...
  if (!replay || engine.mode !== 'arcade' || (replay.version !== 2 && replay.version !== 3 && replay.version !== 4 && replay.version !== 5)) return current;
...
export function readGhost(raw: string | null): GhostRecord | null {
  ...
  try {
    ...
    if (!replay || (replay.version !== 2 && replay.version !== 3 && replay.version !== 4 && replay.version !== 5) || (replay.version >= 3 && replay.mode !== 'arcade') || !Number.isInteger(replay.seed) || replay.seed < 0 ||
