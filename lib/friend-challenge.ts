...
export type FriendChallenge = { version: 2 | 3; seed: number; mode: GameMode; floor: number; score: number };

export function decodeChallenge(value: string | null): FriendChallenge | null {
  if (!value || value.length > 100) return null;
  const parts = value.split('.');
  if (parts.length !== 5 || !['2', '3'].includes(parts[0]) || !['a', 'p', 't'].includes(parts[2]) || (parts[0] === '2' && parts[2] === 't')) return null;
  ...
  return { version: parts[0] === '2' ? 2 : 3, seed, mode: parts[2] === 'a' ? 'arcade' : parts[2] === 't' ? 'party' : 'practice', floor, score };
}

export function challengeFromRun(engine: TowerEngine): FriendChallenge | null {
  if (engine.status !== 'over' || engine.floor < 1) return null;
  return { version: 3, seed: engine.seed, mode: engine.mode, floor: engine.floor, score: engine.score };
}
