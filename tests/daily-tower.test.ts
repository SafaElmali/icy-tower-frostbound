...
void test('daily dates use UTC and produce deterministic distinct tower seeds', () => {
  ...
  assert.equal(a.mode, 'arcade'); assert.equal(a.version, 9);
  ...
  assert.equal(dailyForDate('2026-09-15', 10), null);
});
...
void test('past shared links pin their date and rules and discard competing challenge parameters', () => {
  ...
  for (const invalid of [null, '', '10.2026-09-15.a', '4.2026-09-15.a', '3.2026-09-15.a', '5.2026-09-15.p', '5.2026-09-15.t', '05.2026-09-15.a', '5.2026-09-31.a', '5.2026-09-15.a.extra', 'x'.repeat(10000)]) assert.equal(decodeDailyTower(invalid), null);
});
...
void test('unlimited retries and recipients always start the same Classic tower', () => {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    startDailyRun(engine, daily);
    assert.equal(engine.mode, 'arcade'); assert.equal(engine.rulesVersion, 9);
...
void test('new daily towers use current rules while archived links and personal bests keep their original rules', () => {
  const legacy = decodeDailyTower('5.2026-09-15.a')!;
  const current = dailyForDate('2026-09-15')!;
  assert.equal(legacy.version, 5); assert.equal(current.version, 9);
