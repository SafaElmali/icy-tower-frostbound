...
function recordClimb(version: 2 | 3 | 4 | 5 | 6 = 6) {
...
void test('starting a rematch reuses the ghost tower and other modes start without it', () => {
  ...
  for (const mode of ['party', 'practice'] as const) {
    ...
    assert.equal(player.mode, mode); assert.equal(player.time, 0); assert.equal(player.rulesVersion, 6);
  }
...
void test('old ghosts cannot pin new runs to older rules and are replaced by a current recording', () => {
  const current = recordClimb(6).engine;
  for (const version of [2, 3, 4, 5] as const) {
    ...
    assert.equal(bestGhost({ ...legacy, floor: 950 }, current)?.replay.version, 6);
  }
});
