...
export function LeaderboardDialog({ open, onOpenChange, run }: { open: boolean; onOpenChange: (open: boolean) => void; run: RunReplay | null }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="leaderboard-card">
    ...
    {run && !submitted && <form className="score-submission" onSubmit={submit}>
      <label htmlFor="leaderboard-name">Put your name on the tower</label>
